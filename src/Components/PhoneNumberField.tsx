...
const PhoneNumberField: React.FC<PhoneNumberFieldProps> = ({
  ...
  const countryCodeChangeHandler = (e: any) => {
    setCountryCodeVal(e.target.value);

    const arr = Array.from(flagRef.current);
    var selectedFlag;
    arr.forEach((ele: any) => {
      if (ele.value === e.target.value) {
        selectedFlag = ele.dataset.countrycode.toLowerCase();
        setCountryFlag(
          `https://flagpedia.net/data/flags/w702/${selectedFlag}.webp`
        );
        return;
      }
    });
  ...
  return (
    <div className="border border-slate-300 flex items-center h-16 my-3 mx-auto w-[90vw] md:w-[35vw] md:mx-0 bg-white">
      <div className="flex items-center w-full">
        <img src={countryFlag} alt="" className="w-8 ml-3" />
        <select
          id="country"
          className="w-0 p-3 border-0 outline-0"
          ref={flagRef}
          onChange={countryCodeChangeHandler}
          defaultValue={countryCodeVal}
        >
        ...
        <div className="p-2 w-20">{`+ ${countryCodeVal}`}</div>
        <input
          type="tel"
          className="quoteprice outline-none rounded p-5 pl-0 placeholder-gray-700 placeholder-gray-700 text-sm text-gray-700 w-11/12"
          value={val}
